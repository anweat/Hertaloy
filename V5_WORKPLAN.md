...
> ⚠️ **这是历史工作计划，按当时的判断记录，不随后续归约改写。**
> 其中两处已被取代，读的时候要知道：
>
> - **Task 4「网关 REQUEST/REPLY 与锁账本」** —— 锁账本已删（第八次归约），
>   换成派生的「未了结的义务」，见 FOUNDATION §9.2
> - **M2「隧道 ∩ traceid 前缀」、P1-7「订阅 scope 相对形式」** —— 隧道与订阅整个删掉
>   （第九次归约），换成别名绑定 + 沿 traceid 向上解析，见 FOUNDATION §8
>
> 当前状态以 `FOUNDATION_V5.md` §16 不变量总表为准。
...
默认从"跑完就删"改成"留着"。理由是多开 agent 时现场看不到就没法查，
工作树没了产出也取不回来。

**代价明说**：留着就会一直涨，而对象回收目前没有设计路径。所以：

- 沙箱落在**状态目录**下而不是系统临时目录 —— 放 tmp 的话一次重启现场就没了，
  而"现场还在"正是留它的全部理由。wsl 例外，它必须住在 Linux 文件系统里。
- 沙箱位置写进**执行观测对象**（`<traceid>/$exec`），于是"哪次执行对应哪个沙箱"
  由版本层回答，不需要第二处记账。
- `status` 报出保留了几个 —— 与其在磁盘里悄悄堆，不如让人看得见。

### 改出来的两个真问题

**其一，executionId 不全局唯一从理论变成了现实。**每个 Runtime 都从 `exec-1` 起，
跑完即删时无所谓，留着就会在共享目录里真的撞名。所以沙箱 id 由
`<traceid>/<executionId>` 拼成。

**其二，确定性路径 + 保留 = 复用脏目录。**同一个 id 第二次分配撞上上次的残留，
而基线是在物化之后打的 —— 残留被算进基线，agent 这次真改的东西反而 diff 不出来。
**现有的 profile 测试直接炸了**，抓得很准。修法是 `allocate(id)` 先清空再用：
同 id 就是同一次执行，后来者是重跑，顶掉旧的是**响的**失败，比错误的观测好。

### 顺带补上的契约不一致

`store.put` 只禁 `@`，而 `REF_PATTERN` 连 `$` 都不认 —— 于是 `$run` / `$exec`
这些内核保留对象**存得进去却引用不到**，`hertaloy show job-1/$exec@1` 报"非法引用"。
`$` 做保留前缀是对的（用户资产名是 `Ident`，不含 `$`，撞不上），该改的是 `Ref`。
契约两端对不上账，与 K5、E1 是同一类。

    ...
    expect(
      stateProblems(
        facts({
          ...
          executions: [
            { executionId: "exec-1", traceid: "job-1", status: "RUNNING", claimed: ["msg-1"] },
          ],
        }),
      ),
    ).toEqual([]);
  ...
  it("记录引用了一条不存在的消息 → 不报（那是 GC 掉的历史，不是半状态）", () => {
    expect(
      stateProblems(
        facts({
          executions: [
            { executionId: "exec-1", traceid: "job-1", status: "RUNNING", claimed: ["msg-9"] },
          ],
        }),
      ),
    ).toEqual([]);
  ...
  it("两条 RUNNING 记录抢同一条消息 → 报", () => {
    const out = stateProblems(
      facts({
        ...
        executions: [
          { executionId: "exec-1", traceid: "job-1", status: "RUNNING", claimed: ["msg-1"] },
          { executionId: "exec-2", traceid: "job-1", status: "RUNNING", claimed: ["msg-1"] },
        ],
      }),
    );
    ...
    expect(
      stateProblems(
        facts({
          messages: [msg("msg-1", "CLAIMED")],
          executions: [
            { executionId: "exec-1", traceid: "job-1", status: "SETTLED", claimed: ["msg-1"] },
            { executionId: "exec-2", traceid: "job-1", status: "RUNNING", claimed: ["msg-1"] },
          ],
        }),
      ),
    ...
    expect(
      stateProblems(
        facts({
          messages: [msg("msg-1", "CLAIMED")],
          executions: [
            { executionId: "exec-1", traceid: "job-1", status: "VOIDED", claimed: ["msg-1"] },
            { executionId: "exec-2", traceid: "job-1", status: "RUNNING", claimed: ["msg-1"] },
          ],
        }),
      ),
  ...
  it("还有在途消息 → 报（QUEUED 与 CLAIMED 都算）", () => {
    const out = stateProblems(
      facts({
        ...
        executions: [
          { executionId: "exec-1", traceid: "job-1/c1", status: "RUNNING", claimed: ["msg-2"] },
        ],
      }),
    );

 ...
 *
 * 这是 RENDERING.md §4 那条"场景是纯函数"的落点。`activity` 与 `certainty`
 * 都需要"最近一段历史"，窗口就是本文件的 `WINDOW`（`messages.slice(-WINDOW)`）。
 *
 * 这里原来写的是"靠一个巧合般的合拍"：head 里恰好留着最近 200 条已消费消息
 * （`keepConsumedMessages`），于是染色窗口就是那 200 条。**那个耦合是真的，
 * 也真的不该有** —— 一个持久化 GC 旋钮在暗中决定渲染语义，调小它画面就变。
 * V6 阶段 5 之后终态消息全部落库，快照给的是完整历史，窗口只由这里说了算。
 *
 * ⚠️ 因此 `snapshot.messages` **必须按投递序**（内核的 `Runtime.messages()`
 * 负责，见那儿的注释）：落库那半是按节点分组取回来的，不排的话
 * `slice(-WINDOW)` 取到的是"最后一个节点的 N 条"。
 *
 * 前端仍然一条历史都不用自己存。三条硬性质不变：
 ...
 * 里拼 —— 那条重建路径被证伪过两次（首次失败显示 idle、消息被回收后旧失败盖掉新成功），

...
| | `edges` | 别名绑定（`bindings`）|
|---|---|---|
| 语义 | 内网边，M1 唯一寻址权威 | 网关，别名 → 端点 |
| 两端 | 注册期全查 | **注册期全查**（绑定必须绑到已声明的端点，根上一个不欠）|
| 可判定性 | 可判定 | **绑定存在性可判定；具体是哪个实例不可判定** |

> ⚠️ **这一格比原稿收紧了。**原稿写的是"隧道查不了 —— 原理上不可判定"，
> 依据是 `scope: "$self_subtree"` 要匹配的实例注册时还没被创建。
> 别名归约（FOUNDATION §8）之后要分成两半：
>
> - **绑定存在性**是静态的 —— 注册期就拒"发出去没人接"，画布**画得出**这条声明过的去向
> - **落到哪个实例**仍不可判定 —— 绑定指向子槽，槽里的实例还没创建
>
> 所以"自动审核是否固定"的答案是：边全定；别名**一端定、一端不定**。
> 那正好就是 §2.3 说的"两个确定度"，只是不定的那一半比原稿小了一圈。
...
**边和别名是同一种元素的两个确定度。** 这是这份设计里最实的一条归约：

| | certainty | 何时确定 |
|---|---|---|
| 边 | 恒为 `1` | 注册期证实（两端存在、方向对、契约正文是契约） |
| 别名 | `0..1`，靠命中累积 | 声明在注册期已定，**落到哪个实例**只能观察 |

于是"染色渐显、多次命中后固化成假边"不是一种特效，
**是 certainty 这个通道的可视化本身**。渲染器不需要知道边和别名的区别。

一条从没命中过的别名绑定 certainty = 0，但**仍然要画**——目标端点是静态的，
所以画成一条挂在端点上的须。"声明了但从没人往这儿发"是很值钱的观察。

别名校验只要求"用到的必须绑上"，**反过来不要求** —— 绑了却没人发是合法的，
那根须因此表达得出来。运行期的 `dangling`（emit 了但解析出 0 个目标）仍然是观测，不是错误。
...
export interface Flow {
  readonly id: string;
  readonly from: AnchorRef | null;   // ★ 别名的源端可能未知 —— 见 §5
  readonly to: AnchorRef;
  readonly certainty: number;        // 0..1：边恒 1，别名靠命中累积
...
`Message` 原本有 `target` 和 `alias`（`routing.ts` 真的在填），但**没有来源**：

| 想画的 | 补之前 |
|---|---|
| 某个别名的某个目标端点被命中了几次 | ✅ `alias` + `target` 够了 |
| **命中是从哪个实例来的** | ❌ 完全算不出 |
...
> **⚠️ 气球那一版已被否掉。** 见 §6.1 —— 它对嵌套只给了个弱线索，
> 对生命周期一条视觉轴都没有。下面这段留着记录当时的推理。
>
> **外观基调是气球，不是卡片。** 整体随性、浮动，而不是钉死的网格。
这条定下来之后有个顺手的结果：**`certainty` 直接就是刚度**。

- 边（certainty = 1）把两个 Cell 稳定地拴在一个距离上
- 别名（certainty 低）让两端飘，命中越多拴得越紧 —— 这就是"逐渐固定显现"
- `pinned` = 完全不飘
...
### 6.1 换成「层积」——第一版气球为什么不成立

气球好看，但它是**装饰**不是**结构**：

| | 气球 | 层积 |
|---|---|---|
| 嵌套 | 圈套圈，弱线索 | **几何包含**，最强的那种 |
| 生命周期 | **一条轴都没有** | 带的长度 |
| 时间 | 不出现 | 横轴就是消息序号 |
| 执行有没有宽度 | 看不出来 | agent 是一段，同步是一个刻点 |

换成**层积**：横轴是消息序号（不是墙钟 —— 诚实对应 L0），纵向是嵌套，
每个实例是一条带，子实例的带长在父带里面。

三个收获，都不是"更好看"而是"更真"：

1. **包含关系就是那条不变量。** 父容器必须等所有子实例终止才能收口，
   所以父的生存期**必然**包含子的 —— 图上的几何包含不是排版巧合，
   是不变量的可视化。渲染时用几何断言验过：10 条节点带全部落在实例块内。
2. **「现在」要单独占一格。** 不留这一格的话，"在最后一个序号终止"和
   "还开着"占的宽度一模一样，生命周期那条轴当场作废一半。
3. **同步节点不亮，从缺陷变成特性。** §6 原本把它当作要接受的遗憾；
   在长度轴上它直接成了信息 —— 有宽度 = 真的在外面跑，一个刻点 = 内核里
   一瞬间的事。

配色规矩不变（色相=身份、明度锁死），但**低饱和填充 + 满饱和左沿**比球体
渐变耐看得多：边缘承担识别，填充保持安静。异常仍然不靠色相 ——
色相已被身份占满，failed 用语义红的断口。

**颜色走 style 不走属性。** SVG 属性里的 `var()` 不解析，第一版渲染时把
CSS 变量读成具体值写死，结果主题一换 SVG 还留在上个主题。走 `style`
让浏览器自己解析变量，明暗切换实时跟上、不用重画。
...
- **隐形加宽命中路径**（Node-RED，20px 透明路径包细线）—— 成本极低收益极高，
  而我们**尤其**需要：低 certainty 的别名线很淡，没有加宽根本点不中
...
| 节点能力声明 | contracts + sandbox | ✅ **唯一一件** |

### 9.1 染色边不需要工具集

工具调用的正常结局就是**变成一条真消息**，而消息命中 scene 层现在就吃得下。
工具让 agent 多一条产生这些消息的路径，但不是边能画的前提。

### 9.2 工具集也能零内核改动

关键是**内核只见到 `ExecutionResult`，而沙箱拥有契约目录**：

```
agent  →  hertaloy emit out '{...}'  →  写 .hertaloy/journal.jsonl
沙箱   →  读 journal → 合成 emissions + 整条 journal 进 diagnostics
内核   →  照旧收一个 ExecutionResult，照旧把 diagnostics 写进 $exec
```

`diagnostics` 是 `JsonObject` 自由形状，失败执行也落 `$exec`。
**零部分提交天然保住** —— 合成在沙箱侧，结果仍是一次性交回，
三段式那段不持锁的执行期一点没碰。

⚠️ journal 必须**序号化追加**，不能内容寻址：两次相同的 `emit` 会被去重合并。
这个坑咬过三次。

### 9.3 能力的声明与能力的使用要分在两个面

| | 走 hertaloy 工具？ | |
|---|---|---|
| 编排操作 `emit` `publish` `put` `read` `collect` `spawn` | ✅ | 本来就是 `HandlerContext` 有的，agent 该对等 |
| 沙箱 / 网络策略 | ❌ | 那是**关于** agent 的配置。给了它就等于让它松开自己的笼子 |
| 模板（DDL） | ❌ 不从沙箱里 | 同理：改自己的模板 = 改自己的笼子 |

所以沙箱与网络不做成工具，做成**声明**（`AgentSpec.capabilities`）——
可读、可渲染、agent 碰不到。

### 9.4 进度只有一半能推

- **结构性进度**（可推，已做）：子槽 `已终止 / 已创建`、实例 `碰过的节点 / 模板节点数`
- **语义进度**（推不出来）：「我在跑第 3 组测试」。**原则上**推不出 ——
  有环的流程没有分母。只能声明或上报，那是工具集的活

⚠️ 实例那条是**覆盖率不是完成度**：循环会反复碰同一批节点，分子早到顶而分母不动。

...
>   `nodeflow_*` 模块，测试 `python -m pytest -q`（339 条），
>   复杂度见 [COMPLEXITY.md](./COMPLEXITY.md)。
> - **下一阶段 = V5 全栈（TypeScript 重写 + 真实画布 + 完整后端）**：
>   架构与分阶段计划见 [ARCHITECTURE_FULLSTACK_V5.md](./ARCHITECTURE_FULLSTACK_V5.md)；
>   单文件自包含的整体构建提示词见 [PROMPT_FULLSTACK_BUILD.md](./PROMPT_FULLSTACK_BUILD.md)。
>   已确认决策：全 TS、自研 SVG 画布、最小闭环优先、SQLite→Postgres 预留；
>   Python V4 冻结为黄金 oracle，行为证据继续保留。

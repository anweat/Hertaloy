...
> 配套：`FOUNDATION_V4.md`（为什么这么设计）、`nodeflow_v4.py`（当前实现）、`test_foundation_v4.py`（52 条验收）
>
...
`exit.endpoint` 声明子流程回程端口（§6 #9 已收口）：注册期校验其在子模板
存在；运行期 REPLY 经该端口回父图；未声明时兼容退回 `"reply"` 默认。
...
| **内核工具桥** | ✅ | driver 执行中回调编排面执行 `read_artifact`/`publish`/`spawn` | 能力由节点 `publish_topics`/`spawn_slots` 编译期声明；`kernel_tool` ⇄ `kernel_tool_result` 线协议（`test_kernel_hardening.K`） |
...
| ~~8~~ | ~~tool 注入与命名冲突~~ | — | ✅ 内核保留名 `emit/read_artifact/publish/spawn` 不可占用；跨卡片重名显式报错，不靠加载顺序；工具带 `source` 溯源。内核工具现由节点声明推导并经内核工具桥执行（§5） |

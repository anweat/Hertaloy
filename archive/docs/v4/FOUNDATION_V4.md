...
> **下一阶段 V5 全栈（TS 重写 + 真实画布 + 完整后端）见 [ARCHITECTURE_FULLSTACK_V5.md](./ARCHITECTURE_FULLSTACK_V5.md)；V4 冻结为黄金 oracle。**
...
全仓当前 **339 条测试**：常规全量 321 passed / 18 skipped（其中 `TestPiReal` 8 条
在未设 `PI_REAL=1` + `DEEPSEEK_API_KEY` 时跳过；真实 key 下实测 5/3，见
`HARNESS_EVALUATION.md §3.1`）。

Phase 5 内核加固（2026-08-15，批判性审查轮，证据 `test_kernel_hardening.py` 15 条）：

- **CLOSED 是终态**：`control(pause/resume)` 对 CLOSED 一律拒绝，重复 close
  幂等且不留第二条事实；修复了 close→pause→resume 复活实例的漏洞（S1–S3）。
- **模型 evaluator 提交气密**：claim/execute/apply 三段式与 agent 同规 ——
  执行期间实例 close 或同节点出现更新 claim 时，结果作废、输入 FAILED，
  绝不往 CLOSED 实例路由死信（M1）。
- **持久化 × 并发**：`SqlitePersistence` 改 `check_same_thread=False` +
  写事务锁；`drain_concurrent(workers=N)` 与落盘可以同时工作（P1）。
- **claim/release 是持久提交**：新增 `Runtime.on_change` 钩子，claim 的
  RUNNING 记录 + CLAIMED 消息立即落盘；不再需要测试手动 `flush` 才能
  "崩溃接管"（C1）。持久化同时修正 `unsubscribe` 删除语义，重启后无幽灵
  订阅（U1）。
- **失败路径统一**：backend 抛异常进入 attempts/on_error 重试通道；
  plain/strategy 受信 handler 的运行时异常转为 FAILED + 失败快照，不再让
  drain 裸抛或消息滞留 CLAIMED；subflow 遇 CLOSED child 从 InvariantError
  改为可恢复失败通道（H1–H3；D6 语义同步修订）。
- **目标入口校验**：`send`/`subscribe` 在入口校验实例/节点/端点，LLM 可读
  错误，不再拖到调度时才 KeyError；CLOSED 实例不得新增订阅（T1/T2）。
- **内核工具桥**：`read_artifact`/`publish`/`spawn` 由节点声明
  （`publish_topics`/`spawn_slots`）在编译期定型并进 `prefix_hash`；
  driver 执行中发 `kernel_tool` 行回调编排面，`SubprocessBackend` 应答
  `kernel_tool_result`；`fake/openai_compat/pi` 三个 driver 均已接线
  （K1–K3）。Agent 首次可以真实地读产物、发 topic REQUEST、实例化子容器。
...
18. ~~内核 P0 修复（审查轮）~~ ✅ 终态守卫 / 模型策略提交气密 / 持久化线程安全与
    claim 落盘 / 退订持久化 / 异常失败路径 / 目标校验 / 内核工具桥；
    `test_kernel_hardening.py` 15 条钉住（§10 Phase 5）
...
- 审查轮 P1 收尾：定义层注册加锁与角色矩阵、卡片深只读、WARM_POOL 清 tail、
  ExecutionLimits 的 wall_clock/max_tool_calls 执行、快照与 seq 语义定稿、
  投影返回副本。这些不破坏主链，但进入真实后端前需逐项关闭。

...
function buildTools(req, executionId, emissions) {
  ...
  // 内核工具桥：read_artifact/publish/spawn 回调编排面（P0-6）
  const kernel = (req.agent_spec?.kernel_tools ?? []).map((t) => ({
    ...
    executionMode: "sequential",
    execute: async (_id, params) => {
      const result = await callKernelTool(executionId, t.name, params ?? {});
      return {
  ...
  return {
    tools: [emitTool, ...declared, ...kernel],
    kernelNames: new Set(kernel.map((t) => t.name)),
  };
}
...
async function handleRun(req) {
  ...
  const agent = new Agent({
    ...
    beforeToolCall: async ({ toolCall, args }) => {
      ...
      const isKernel = kernelNames.has(name);
      const gated = name === "emit" || declaredNames.has(name) || isKernel;
      observations.push({
        kind: "tool_call",
        name,
        gated,

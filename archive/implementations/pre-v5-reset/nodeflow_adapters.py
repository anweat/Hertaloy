...
driver → Python（stdout，每行一个 JSON）：
    ...
    {"type": "kernel_tool", "execution_id": ..., "tool_call_id": ...,
                            "name": ..., "arguments": {...}}        # 内核工具回调
    {"type": "result",      "execution_id": ..., "result": {...}}
    {"type": "error",       "execution_id": ..., "message": "..."}

Python → driver（stdin）对 kernel_tool 的应答：
    {"type": "kernel_tool_result", "execution_id": ..., "tool_call_id": ...,
     "result": {...}}                          # 成功
    {"type": "kernel_tool_result", "execution_id": ..., "tool_call_id": ...,
     "error": "..."}                           # 失败
...
一个合法 driver 必须：
  ...
  3. 工具封闭        —— 暴露给模型的工具集 == agent_spec.tools ∪ kernel_tools ∪ {emit}，一个不多
  ...
  8. 内核工具回调    —— kernel_tools 中的工具不得本地臆造；必须发 kernel_tool
                      行并等待 kernel_tool_result 后把结果回传给模型
"""
...
def request_to_json(req: ExecutionRequest) -> dict[str, Any]:
    return {
        ...
        "context": {
            ...
            "meta": [dict(m) for m in req.context.meta],
        },
...
def json_to_request(payload: Mapping[str, Any]) -> ExecutionRequest:
    ...
    return ExecutionRequest(
        ...
        context=InvocationContext(
            ...
            meta=tuple(dict(m) for m in ctx.get("meta", ())),
        ),
...
class SubprocessBackend(ExecutionBackend):
    ...
    def __init__(
        ...
        *,
        ...
        #: 内核工具桥：driver 发出 kernel_tool 行时，编排面用此 handler 执行
        #: read_artifact/publish/spawn（Runtime.set_backend 注入）。
        self.kernel_tool_handler: Callable[[str, str, Mapping[str, Any]], Any] | None = None
        ...
        try:
            while True:
                ...
                if kind in ("event", "observation"):
                    ...
                    continue
                if kind == "kernel_tool":
                    # 内核工具桥（P0-6）：driver 执行中回调编排面，
                    # 结果/错误以 kernel_tool_result 行写回同一 driver。
                    call_id = msg.get("tool_call_id")
                    try:
                        if self.kernel_tool_handler is None:
                            raise DriverError("no kernel tool handler bound")
                        result = self.kernel_tool_handler(
                            eid, msg.get("name", ""), msg.get("arguments") or {})
                        payload = {"type": "kernel_tool_result",
                                   "execution_id": eid,

...
class TestSubflowReuse(FoundationTestCase):
    ...
    def test_D6_call_style_child_close_does_not_orphan_parent(self):
        """帧 9：子实例关闭后，父调用成为可观测失败，不再制造永远等待的子调用。

        修订（内核加固 P0-7）：失败走 handler 失败通道（FAILED + 失败快照），
        drain 不再把异常裸抛给调用方。
        """
        ...
        self.rt.drain(job)          # 不再 raise

        # 父输入进 FAILED 终态；没有新子消息/下游路由，可观测失败快照存在
        parent_msgs = [m for m in self.rt._messages.values()
                       if m.target[0] == job]
        self.assertTrue(any(m.state == "FAILED" and m.payload == {"round": 1}
                            for m in parent_msgs))
        self.assertFalse(any(m.state in ("QUEUED", "CLAIMED")
                             for m in parent_msgs))
        # 第 1 轮没有沿边流到 sink（sink 仍只有第 0 轮产物）
        self.assertEqual(self.rt.node_persistent_state(job, "sink")["last"],
                         {"reviewed": {"round": 0}})
        snaps = self.rt.store.history(f"run/{job}")
        self.assertTrue(any("failure" in s.body and "HANDLER_FAILED" in str(s.body["failure"])
                            for s in snaps))

...
class DefinitionsMixin:
    ...
    def prefix_fingerprint(spec: Mapping[str, Any]) -> str:
        ...
        blob += json.dumps(
            spec.get("kernel_tools") or [], sort_keys=True, ensure_ascii=False,
            default=repr)
    ...
    def _validate_template(self, template_id: str, spec: Mapping) -> None:
        ...
        for node_id, node in nodes.items():
            ...
            # 内核工具桥能力只在 agent 节点上声明；其它 kind 声明即拒，
            # 避免"配置了却静默不生效"（第一不变量：能力必须显式、可见）。
            if kind != "agent" and (node.get("publish_topics") is not None
                                    or node.get("spawn_slots") is not None):
                raise InvariantError(
                    f"节点 {node_id!r}（kind={kind}）不得声明 publish_topics/"
                    f"spawn_slots：内核工具桥只对 agent 节点开放")
            if kind == "agent":
                ...
                # 内核工具桥（P0-6）：能力必须在编译期声明完整。
                # publish 只能选已声明 topic，回调只能落回本节点已声明端点；
                # spawn 只能实例化已声明 slot —— Agent 不构造地址/能力。
                pt = node.get("publish_topics")
                if pt is not None:
                    if not isinstance(pt, Mapping):
                        raise InvariantError(
                            f"节点 {node_id!r} 的 publish_topics 必须是 "
                            f"{'{topic: callback_endpoint}'} 映射")
                    for topic, cb_ep in pt.items():
                        if topic not in self._topics:
                            raise InvariantError(
                                f"节点 {node_id!r} 的 publish_topics 引用了未注册 "
                                f"topic {topic!r}；先 register_topic。"
                                f"可用 topic：{sorted(self._topics)}")
                        _ep_exists(node_id, cb_ep,
                                   where=f"节点 {node_id!r} 的 publish_topics[{topic}]")
                ss = node.get("spawn_slots")
                if ss is not None:
                    if not isinstance(ss, (list, tuple)):
                        raise InvariantError(
                            f"节点 {node_id!r} 的 spawn_slots 必须是 slot id 列表")
                    for slot_id in ss:
                        if slot_id not in slots:
                            raise InvariantError(
                                f"节点 {node_id!r} 的 spawn_slots 引用了未声明 "
                                f"slot {slot_id!r}。可用 slots：{sorted(slots)}")
            elif kind == "plain":

...
class SchedulingMixin:
        ...
        try:
            if unit.kind == "agent":
                ...
                try:
                    ...
                    # backend 本身抛异常（网络/驱动崩溃）也是一次失败：
                    # 进入重试与 on_error 通道，绝不滞留 CLAIMED/RUNNING。
                    with self._lock:
                        self._release(unit.execution_id, "FAILED", reason="FAILED")
                    return
                with self._lock:
                ...
                with self._lock:
                    if result.termination != "DONE":
                        self._release(
                            ...
                            reason=result.termination)
                    elif (self._instances[rec.gid].status != "OPEN"
                          or self._instances[rec.gid].nodes[rec.node_id].version
                          != rec.base_node_version):
                        # 终态气密 + 节点级冲突域：模型 evaluator 与 agent 同规。
                        # 执行期间实例被 close，或同节点出现了更新的 claim，
                        # 本结果作废，输入进 FAILED 终态，绝不往 CLOSED 实例路由。
                        self._release(unit.execution_id, "FAILED",
                                      reason="APPLY_REJECTED")
                    else:
                        try:
                ...
                try:
                    ...
                    # 契约/evaluator 守门违规是**确定的编程错误**，保持既有
                    # 行为：立刻抛给调用方，不进失败重试通道。
                    raise
                except Exception as exc:
                    with self._lock:
                        self._fail_unit_messages(unit, exc)
            else:
                try:
                    with self._lock:
    ...
    def _fail_unit_messages(self, unit: _Unit, exc: BaseException) -> None:
        """受信 handler 抛异常：消息进 FAILED 终态 + 失败快照，不滞留 CLAIMED。

        与 agent 路径的 _release 对齐：异常是失败，必须可观测、可进图，
        不能把 drain 变成裸异常传播（P0-7）。
        """
        msgs = (
            [unit.msg]
            if unit.kind == "simple"
            else list(unit.batch) + list(unit.discard)
        )
        for m in msgs:
            if m.state in ("QUEUED", "CLAIMED"):
                m.state = "FAILED"
        reason = f"HANDLER_FAILED:{type(exc).__name__}:{exc}"
        self._raise_into_graph(unit.inst, unit.node_id, unit.node, msgs,
    ...
    def _handle_subflow(self, inst, tpl, node_id, node, msg) -> dict[str, Any]:
        ...
        if self._instances[child].status != "OPEN":
            ...
            # 用 RuntimeError 而非 InvariantError：这是**可恢复运行故障**，
            # 应进入 handler 失败通道（P0-7），而不是把 drain 变成契约错误裸抛。
            raise RuntimeError(f"bound child {child} is not OPEN")
        entry_node, entry_ep = slot["entry"].split(".")
    ...
    def _claim(self, inst, node_id, node, msgs, *, spec_id=None) -> str:
        ...
        resolved["kernel_tools"] = self._kernel_tool_defs(node)
        ...
        # claim 是提交 A：RUNNING 记录 + CLAIMED 消息 + seq 推进必须在此刻落盘，
        # 否则 claim→apply 之间崩溃会静默重跑执行面的外部副作用（P0-4）。
        self._notify_change()
        return eid
    ...
    def _release(self, execution_id, status, *, reason=None) -> None:
        ...
        if status == "CANCELLED":
            ...
            self._notify_change()           # CANCELLED 事实也要持久化（P0-4）
            return
        ...
        if retryable and all(m.attempts < cap for m in msgs):
            for m in msgs:
                m.state = "QUEUED"          # 还能再试
            self._notify_change()           # 重试计数与回队状态要持久化（P0-4）
            return

...
- **孤儿认领只把窗口关小到一次驱动内**。两个并发驱动仍可能互相认领对方的
  在途执行 —— 真修法是执行租约，未做。见 FOUNDATION §21.2。
- **`ref` 变量类型声明了但没有解引用**。到 agent 手里是字符串，
  B1 的运行期上界对它恒为真空。见 §21.5。
- **帧 14（上下文不随轮次膨胀）没有强制点**，全靠模板作者自觉。

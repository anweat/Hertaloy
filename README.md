...
批 0（事务化提交）、A（资产 ctx）、F（对象命名空间）、B（物化继承）、C（权限）已落地。

```
packages/contracts   51 条   身份/路径/变量/端口/消息/契约/模板/执行面 schema
packages/kernel     113 条   store · tx · instances · locks · context · extract · routing · runtime · control
合计                164 条   typecheck 绿，离线，无数据库/网络/真实模型
